...
$langs->loadLangs(array("mails", "bills"));

$id = (GETPOST('facid', 'int') ? GETPOST('facid', 'int') : GETPOST('id', 'int'));
$action = GETPOST('action', 'alpha');
$option = GETPOST('option');
$mode = GETPOST('mode');
$toGenerate = GETPOST('toGenerate', 'array:aZ09') ?? null;
	...
	if (!$error) {
		...
		for ($i = 0; $i < $countToSend; $i++) {
			...
			if ($result > 0)    // Invoice was found
			{
				...
				$filedir = $conf->invoice->dir_output . '/' . dol_sanitizeFileName($object->ref);
...
if ($action == "builddoc" && $user->hasRight('facture', 'lire') && !GETPOST('button_search') && !empty($builddoc_generatebutton)) {
	if (is_array($toGenerate) && !empty($toGenerate)) {
		...
		foreach ($toGenerate as $tmppdf) $arrayofinclusion[] = preg_quote($tmppdf . '.pdf', '/');
		...
		foreach ($toGenerate as $basename) {
		...
		$outputlangs = $langs;
		$newlang = '';
		if (getDolGlobalInt('MAIN_MULTILANGS', 0) && empty($newlang) && GETPOST('lang_id')) {
			$newlang = GETPOST('lang_id');
		}
		if (getDolGlobalInt('MAIN_MULTILANGS', 0) && empty($newlang)) {
			$newlang = !empty($object->thirdparty->default_lang) ?
				$object->thirdparty->default_lang :
				(!empty($object->client->default_lang) ? $object->client->default_lang : '');
		}
		if (!empty($newlang)) {
		...
		if (getDolGlobalInt('MAIN_DISABLE_PDF_COMPRESSION', 0)) {
		...
		// We can't passed $TtoGenerate to call write_file after it's not in signature of the standard write_file function
		$object->TtoGenerate = $TtoGenerate;
		$result = lcr_pdf_create($db, $object, 'generic_lcr', $outputlangs, $hidedetails ?? 0, $hidedesc ?? 0, $hideref ?? 0);
...
//		}
		...
		dol_syslog($langs->trans("LCR_SELECTINVOICE"), LOG_INFO);
		setEventMessage($langs->trans("LCR_SELECTINVOICE"), 'warnings');
	}

} elseif (is_array($toGenerate) && isset($_REQUEST['generateCSV'])) {
	generateCSV();
}
...
$sql .= " , f.ref_client, f.increment, f.total_ht as total_ht, f.total_tva as total_tva, f.total_ttc, f.localtax1, f.localtax2, f.revenuestamp";
...
if ($search_montant_ht) $sql .= " AND f.total_ht = '" . $db->escape($search_montant_ht) . "'";
...
$sql .= " GROUP BY s.nom, s.rowid, s.email, f.rowid, f." . $facref . ", f.increment, f.total_ht, f.total_tva, f.total_ttc, f.localtax1, f.localtax2, f.revenuestamp, f.datef, f.date_lim_reglement, f.paye, f.fk_statut, f.type ";
if (!$user->hasRight('societe', 'client', 'voir') && !$socid) $sql .= ", sc.fk_soc, sc.fk_user ";
$sql .= " ORDER BY ";
...
if ($resql) {
	...
	print_liste_field_titre($langs->trans("Taxes"), $_SERVER["PHP_SELF"], "f.total_tva", "", $param, 'align="right"', $sortfield, $sortorder);
	...
	if (empty($mode)) {
		...
		//echo $filedir;exit;
		print $formfile->showdocuments('lcr', '', $filedir, $urlsource, $genallowed, $delallowed, '', 1, 1, 0, 48, 1, $param, $langs->trans("LCR_GENERATEFILE"), $langs->trans("LCR_FUSIONLCR"));
		?>
